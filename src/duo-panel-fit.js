(function(){
  ...
  function columnFill(el, plh, cols) {
    ...
    var st = { full: [], orphan: [] };
    for (var c = 0; c < cols; c++) { st.full[c] = false; st.orphan[c] = false; }
  ...
  function fit(panel) {
    ...
    group.forEach(function(el){
      ...
      if (el.classList.contains('card-preview-block')) {
        ...
        if (colCount > 1) {
          // Multi-column essay excerpt (two on the squares, three on the
          // extra-wide split cells): line-clamping individual paragraphs
          ...
          // and fills with as many whole lines as fit; the sub-line
          // remainder (and whatever a short excerpt leaves) moves ABOVE
          // THE TITLE via the slack padding at the end of fit(), so the
          // last line still ends exactly GAP_BOTTOM above the band.
          ...
          var blockLines = pickColumnHeight(el, plh, maxLines, colCount);
          if (blockLines) {
        ...
        // Single-column preview (trio/quad/mosaic squares and the split
        // rows' postscript thirds): paragraphs clamp at the rendered
        // line inside scGap's budget — as many whole lines as fit.
        //
        // Positions are COMPUTED, not read: with every paragraph's clamp
        // lifted to 999 for measuring, sibling clamp boxes in some
        // engines (the in-app preview among them) stop stacking — all of
        // them sit at the block's top and rect.top lies by a whole
        // paragraph. Heights stay true, so the real layout is block top
        // plus a running sum of heights and margins. That fed one card a
        // clamp of 12 where 2 fit ("Curtis Yarvin Jr.", 181px past the
        // band) and let every other card's paragraphs "fit" unclamped —
        // which is why no ellipsis ever appeared.
        var scGap = 16;
        var scLimit = (limit + 14) - scGap;
        var parasCut = false;
        var lastVisible = null;
        var scY = el.getBoundingClientRect().top;
        var scFirst = true;
        [].forEach.call(el.querySelectorAll('.card-preview'), function(p){
          if (parasCut) { p.style.display = 'none'; return; }
          var pTop = scY + (scFirst ? 0 : (parseFloat(getComputedStyle(p).marginTop) || 0));
          scFirst = false;
          var pBottom = pTop + p.getBoundingClientRect().height;
          if (pBottom <= scLimit) { lastVisible = p; scY = pBottom; return; }
          parasCut = true;
          // clampToFit's own rect would reuse the lying top — inline the
          // same clamp against the computed one.
          var pLh = parseFloat(getComputedStyle(p).lineHeight) || 22;
          var pLines = Math.floor((scLimit - pTop) / pLh);
          if (pLines < 1) { p.style.display = 'none'; return; }
          p.style.display = '-webkit-box';
          p.style.webkitBoxOrient = 'vertical';
          p.style.overflow = 'hidden';
          p.style.webkitLineClamp = String(pLines);
          p.style.lineClamp = String(pLines);
