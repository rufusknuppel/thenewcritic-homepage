(function(){
  ...
  // column is absolutely bound to its grid area, so text that runs past
  // it is cut from the bottom up: hard blocks hidden, running text
  // clamped to the lines that fit (the same measure-then-clamp approach
  // as duo-panel-fit.js, with the same drop-cap-preserving max-height
  // path for the opening paragraph).
  function fitCard(card) {
    var text = card.querySelector('.arch-ledger-card-text');
    if (!text || card.hidden) return;
    var readon = text.querySelector('.arch-ledger-readon');
    var paras = text.querySelectorAll('.card-preview');
    [].forEach.call(text.children, function(el){
      if (el !== readon) el.style.display = '';
    });
    [].forEach.call(paras, function(p){
      ...
      p.classList.remove('card-preview--capped');
    });
    ...
    var limit = (readon
      ? readon.getBoundingClientRect().top
      : text.getBoundingClientRect().bottom - 32) - 14;
    function clampToFit(el) {
      var r = el.getBoundingClientRect();
      var lh = parseFloat(getComputedStyle(el).lineHeight) || 22;
      var lines = Math.floor((limit - r.top) / lh);
      if (lines < 1) { el.style.display = 'none'; return; }
      if (lines >= 2 && el.querySelector('.card-preview-dropcap')) {
        // Keep the floated drop cap: cut by max-height (the cut still
        // lands on a line boundary); the ellipsis comes from the
        // .card-preview--capped ::after.
    ...
    [].forEach.call(text.children, function(el){
      ...
      if (el.classList.contains('card-preview-block')) {
        ...
        cutting = true;
        if (!el.classList.contains('card-dek')) { el.style.display = 'none'; return; }
        clampToFit(el);
      }
    });
  }
